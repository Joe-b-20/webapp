...
# a function to save the job postings to a file
import csv
...
def save_jobs_to_csv(jobs, filename):
    with open(filename, 'w', newline='', encoding='utf-8') as file:
        ...
        for job in jobs:
            writer.writerow([job['title'], job['company'], job['location'], job['url'], job['description']])


if __name__ == '__main__':
    test_get_jobs_posts_indeed()
    jobs = get_jobs_posts_indeed(job_title={'name': 'Software Engineer'}, job_location={'name': 'Reston, VA'})
    save_jobs_to_csv(jobs, 'job_postings.csv')
